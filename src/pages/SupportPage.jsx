...
export default function SupportPage() {
  const { items, loading, error } = useRegistry()
      ...
      <main className="overflow-hidden pt-28">
        ...
        <section className="relative px-5 pb-16 text-center sm:px-8 md:pb-20">
          ...
          <div className="relative mx-auto max-w-3xl">
            ...
            <p className="mx-auto mt-6 max-w-lg text-sm font-light leading-relaxed text-sand-100/55">
              As {config.bride} and {config.groom} build their first home together, these are a few
              things they'll treasure. Choose an item or simply send a gift from the heart.
            </p>
            <button
              type="button"
              onClick={() => openGift(null)}
              className="group mt-8 inline-flex min-h-12 items-center gap-3 rounded-full bg-sand px-7 py-3.5 text-sm font-medium uppercase tracking-[0.18em] text-teal-950 shadow-lg shadow-sand/10 transition hover:bg-sand-200"
            >
        ...
        {loading && (
          <div className="py-24 text-center">
            <p className="text-sm font-light text-sand-100/40 animate-pulse-soft">
              Loading registry…
            </p>
          </div>
        )}
        ...
        {error && (
          <div className="mx-auto max-w-md px-5 py-12 text-center">
            <p className="text-sm font-light text-red-300">
              Could not load the registry right now. Please refresh the page.
            </p>
          </div>
        )}
        ...
        {!loading && !error && items.length === 0 && (
          <div className="py-24 text-center">
            <p className="font-display text-2xl font-light italic text-sand-100/50">
              All gifts have been received.
            </p>
            <p className="mt-3 text-sm font-light text-sand-100/35">
              You can still send a gift from the heart using the button above.
            </p>
          </div>
        )}
            ...
            <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 md:py-16">
                  ...
                  return (
                    <article
                      ...
                      className={`group relative cursor-pointer overflow-hidden rounded-[1.5rem] border bg-teal-900/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/30 ${
                        isFeatured ? 'border-sand-300/20 shadow-md shadow-black/10' : 'border-sand-100/10'
                      } ${spanClass}`}
              ...
              <div className="mx-auto mt-16 max-w-2xl border-t border-sand-100/10 pt-12 text-center">
                ...
                <button
                  type="button"
                  onClick={() => openGift(null)}
                  className="mt-7 min-h-11 rounded-full border border-sand-300/30 px-7 py-3 text-xs uppercase tracking-[0.2em] text-sand transition hover:bg-sand/10"
                >
                  See account details
                </button>
              </div>
